...
const displayEsq = document.getElementById("pontosEsq");
const displayDir = document.getElementById("pontosDir");
...
function atualizaAnimacao() {
  ...
  // Raquete Esquerda
  if (teclas['w'] || teclas['W']) tyBE += velocidadeRaquete;
  if (teclas['s'] || teclas['S']) tyBE -= velocidadeRaquete;

  // Raquete Direita
  if (teclas['ArrowUp']) tyBD += velocidadeRaquete;
  if (teclas['ArrowDown']) tyBD -= velocidadeRaquete;
  ...
  if (tyBola + raio > 1.0 || tyBola - raio < -1.0) {
  ...
  if (txBola - raio < -0.85 && txBola + raio > -0.95 &&
    tyBola + raio > tyBE - 0.2 && tyBola - raio < tyBE + 0.2) {
    txBola_offset = Math.abs(txBola_offset);
  }

  if (txBola + raio > 0.85 && txBola - raio < 0.95 &&
    tyBola + raio > tyBD - 0.2 && tyBola - raio < tyBD + 0.2) {
    txBola_offset = -Math.abs(txBola_offset);
  }
  ...
  // Bola saiu pela DIREITA (Ponto da Esquerda)
  if (txBola > 1.0) {
    scoreEsq++;
    displayEsq.innerText = scoreEsq; // Atualiza o HTML
    resetarBola();
  } 
  // Bola saiu pela ESQUERDA (Ponto da Direita)
  else if (txBola < -1.0) {
    scoreDir++;
    displayDir.innerText = scoreDir;
    resetarBola();
  }
  ...
  function resetarBola() {
    ...
    tyBola_offset = (Math.random() > 0.5 ? 1 : -1) * 0.008;
  }
}
